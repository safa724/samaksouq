...
class CheckOut extends StatefulWidget {
  ...
  const CheckOut({Key? key, required this.selectedAddressId, required this.totalAmount}) : super(key: key);
...
Future<void> getUserIdFromSharedPreferences() async {
  ...
  Future<void> fetchDeliverySlots() async {
    final response = await http.get(Uri.parse('http://samaksouq.com/api/v2/deliveryslot'));
    ...
    if (response.statusCode == 200) {
      ...
      if (slotsData['success']) {
        setState(() {
          _deliverySlots = [
            {'date': slotsData['today']['date'], 'day': slotsData['today']['day'], 'slots': slotsData['today']['slots']},
            {'date': slotsData['tomorrow']['date'], 'day': slotsData['tomorrow']['day'], 'slots': slotsData['tomorrow']['slots']},
          ];
        });
      }
    }
        ...
        children: [
          Column(
                        ...
                        Padding(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              border: Border.all(color: Color.fromARGB(255, 6, 85, 150)),
                            ...
                            child: Padding(
                              ...
                              child: Row(
                                children: [
                                  Container(height: 70, width: 100, child: Image.asset('images/card.png')),
                                  SizedBox(width: 10),
                                  Column(
            ...
            if (wantBox) {
              ...
              selectedBoxId = null; // Reset selectedBoxId when checkbox is unchecked
            }
          });
                            },
                            child: Container(
                              width: 470,
                              decoration: BoxDecoration(
                                border: Border.all(color: Color.fromARGB(255, 6, 85, 150), width: 2),
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          Container(
                                            ...
                                            width: 20,
                                            decoration: BoxDecoration(
                                              color: wantBox ? Color.fromARGB(255, 6, 85, 150) : Colors.white,
                                      ...
                                      Column(
                                        ...
                                        children: [
                                          Padding(
                                            ...
                                            child: Align(
                                              alignment: Alignment.centerLeft,
                                              child: Text('please select a box type'),
                                            ),
                                          ),
                                          SizedBox(height: 8),
                                          for (var box in boxes)
                                            GestureDetector(
                                              ...
                                              child: Padding(
                                                ...
                                                child: Container(
                                                  decoration: BoxDecoration(
                                                    color: selectedBoxId == box.id ? Color.fromARGB(255, 9, 82, 141) : Color.fromARGB(255, 85, 105, 122),
                                                    borderRadius: BorderRadius.circular(5),
                                                  ),
                                                  height: 30,
                                                  width: _calculateContainerWidth(box),
            ...
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
          for (Widget container in _buildDateContainers())
            Container(
              margin: EdgeInsets.only(right: 8), // Adjust the right margin as needed
              child: container,
            ),
              ],
            ),
            ...
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
          for (Widget container in _buildTimeContainers())
            Container(
              margin: EdgeInsets.only(right: 8), // Adjust the right margin as needed
              child: container,
            ),
          ...
          Container(
            ...
            child: Center(
              child: CircularProgressIndicator(color: Color.fromARGB(255, 34, 79, 116),), // Loader widget
            ),
          ),
        ],
      ),
      ...
      return GestureDetector(
        onTap: () {
          setState(() {
            ...
            _selectedDate = slot['date']; // Set the selected date.
          });
        },
      child: Container(
        decoration: BoxDecoration(
          color: isSelected ? Color.fromARGB(255, 9, 82, 141) : Color.fromARGB(255, 85, 105, 122),
          borderRadius: BorderRadius.circular(5),
        ),
        height: 33,
        padding: EdgeInsets.all(8),
        margin: EdgeInsets.symmetric(vertical: 4),
        child: Text('${slot['date']},${slot['day']}', style: TextStyle(color: Colors.white)),
      ),
    );
  }).toList();
}
 ...
 List<Widget> _buildTimeContainers() {
    return _selectedSlots.map((slot) {
      final bool isSelected = _selectedTimeSlotId == slot['id']; // Check if the current slot's ID is selected.
      return GestureDetector(
        onTap: () {
          setState(() {
            _selectedTimeSlotId = slot['id']; // Update the selected time slot ID.
          });
        },
        child: Container(
          decoration: BoxDecoration(
          ...
          padding: EdgeInsets.all(8),
          margin: EdgeInsets.symmetric(vertical: 4),
          child: Text('${slot['start_time']} - ${slot['end_time']}', style: TextStyle(color: Colors.white)),
        ),
      );
    }).toList();
  }
...
void _navigateToDeliveryPage() async {
   setState(() {
      _isLoading = true; // Show loader when navigating
    });
  if (_selectedTimeSlotId != null && _selectedDate != null) {
    // Prepare request body for the first API
...
print(accessToken);
    try {
      // Call the first API to create the order
      ...
      if (orderResponse.statusCode == 200) {
        ...
        final Map<String, dynamic> paymentRequestBody = {
          "combined_order_id": jsonDecode(orderResponse.body)['combined_order_id'].toString(),
          "user_id": userId,
        };

        try {
          // Call the second API for payment
          ...
          if (paymentResponse.statusCode == 200) {
            // If payment is successful, navigate to payment screen
            final Map<String, dynamic> paymentData = jsonDecode(paymentResponse.body);
            ...
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to make payment')));
          }
        } catch (paymentError) {
          print('Error making payment: $paymentError');
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error making payment')));
        }
      } else {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Please select a delivery date and time')));
  }
  
    setState(() {
      _isLoading = false; // Hide loader after navigation completes
    });
